...
This document specifies the `did:oan` DID method. It is intended to be read together with the [W3C DID Core Recommendation](https://www.w3.org/TR/did-core/).

This specification defines OAN as a DID **method** for agent-native identity, service discovery, address identification, delegation-aware control, and model-fingerprint-aware bindings in blockchain and cross-domain digital systems.

The latest version of this document is available from the [OAN public documents repository](https://github.com/OpenAgenet/oan-public-docs/tree/main/did-oan-specs).

## 1. Introduction

`did:oan` is a decentralized identifier method for **intelligent agents**, especially software agents, AI service agents, and model-backed agents that need:
...
`did:oan` is optimized for **agent-centric identity**. In this version of the method, the DID subject represents an intelligent agent. It commonly represents:
...
These semantics are first-class method requirements in `did:oan`, not merely optional application conventions.
...
6. Preserve DID Core interoperability while retaining OAN-specific method semantics.

## 3. Why a New DID Method Is Necessary

`did:oan` is introduced because existing general-purpose DID methods do not, by themselves, require or standardize the following semantics that are central to OAN:

1. **Address identification semantics**
   `did:oan` is intended to identify an intelligent agent together with associated address information across chains, domains, or service systems.

2. **Delegation-chain semantics**
   `did:oan` requires support for delegated authority relationships, including delegation scope, delegation proofs, and optional revocation metadata.

3. **Model-fingerprint-aware identity bindings**
   `did:oan` supports identity assertions in which an AI agent, model-backed agent, or agent service deployment may expose a large-model fingerprint or related binding metadata.

4. **Service-discovery semantics**
   `did:oan` treats service discovery and resolver routing as important method-level concerns, especially for multi-network and cross-domain systems.

5. **Native semantic agent-description semantics**
   `did:oan` supports a structured agent-description object directly in the DID Document for agent-native discovery and interoperability.

6. **Recovery-aware control model**
   `did:oan` distinguishes active authentication authority from recovery authority, allowing controlled key replacement without changing the DID itself.

If these properties were left entirely to application-specific extensions, different implementations would not be guaranteed to interpret them consistently. `did:oan` therefore defines them at the method level.
...
`did:oan` is intended for blockchain-integrated and cross-domain digital systems, especially where:
...
The `did:oan` identifier structure is defined as follows:

<img src="image/oan.png" alt="OAN Structure" style="zoom: 67%;" />

In this version of the `did:oan` method, the method-specific identifier consists of two parts:
...
For this version of `did:oan`, the subject category is fixed to `AG`, meaning that the DID subject is an intelligent agent. This version of the method does not define other subject-category codes.

Accordingly, the semantic-code structure in this version is:

- `AG` = agent subject category
- `XX` = a two-character application-domain code defined by the relevant `did:oan` deployment, profile, registry, or governance policy
...
This restriction applies to the primary DID subject defined by this version of the `did:oan` method. Relationship positions appearing inside a DID Document, such as `controller`, delegated counterparties, or externally referenced service operators, MAY also use `did:oan` identifiers, but if they identify non-agent subjects, their subject-category codes SHOULD be distinct from `AG`.

If a broader OAN-family deployment later introduces non-agent DID subjects, those subjects SHOULD continue to follow the same `did:oan` structural pattern while using subject-category codes distinct from `AG`. This version of the method does not define those additional codes in detail, although later examples MAY use such codes illustratively to show how non-agent counterparties can be referenced.

Example structure:

```text
did:oan:AGFI:xxxxxxxxxxxxxxxxxxxxxx
```
...
The OAN identifier generation logic is defined as follows:

1. Select a cryptographic algorithm, such as SM2, Ed25519, or Secp256k1.
2. Generate a public/private key pair.
3. Encode the public key using Base58, Base32, or another OAN-compatible alphanumeric encoding profile. Any encoding used for the suffix MUST produce only letters and digits so that it conforms to the ABNF defined above.
4. Concatenate the prefix `did:oan:`, a four-character semantic code such as `AGFI`, and the encoded public key string to form the DID.

<img src="image/generateOAN.png" alt="OAN Generation"  />
...
| --- | --- |
| Base58 | `f` |
| OAN-compatible alphanumeric encoding profile | `s` |
| Base32 | `t` |

### 8.4 Identifier Semantics

The `did:oan` identifier is method-specific and globally scoped within the `oan` method namespace. The DID itself is immutable. Lifecycle changes affect the DID Document and associated metadata, not the DID string.
...
A conforming `did:oan` DID Document MUST satisfy DID Core requirements. In particular, a valid representation MUST include:
...
Only `@context` and `id` are universally required for the JSON-LD DID Document representation described here. Other DID Core properties remain optional unless additionally constrained by a particular `did:oan` deployment profile or application policy.

### 10.2 Method-Specific Requirements

For `did:oan`, a conforming DID Document SHOULD additionally include a method-specific object named `oanMetadata` when the DID identifies an intelligent agent or an OAN infrastructure node.

The `oanMetadata` object is the primary place for method-specific identity, address, delegation, routing, capability, and OAN lifecycle semantics.

For compatibility with earlier OAN prototype documents that used the historical `did:ans` naming convention, implementations MAY accept `ansMetadata` as a legacy alias during migration. New `did:oan` DID Documents SHOULD use `oanMetadata`.

### 10.3 Method-Specific `oanMetadata`

The `oanMetadata` object MAY contain the following properties:

| Property | Type | Required | Description |
| --- | --- | --- | --- |
| `subjectType` | string | Recommended | Subject type. In this version of the method, this value SHOULD be `agent` for business or user Agents and MAY be `infrastructure-node` for OAN Root, Registrar, Discovery, or related infrastructure roles. |
| `nodeRole` | string | Optional | OAN role such as `root`, `registrar`, `discovery`, `service-agent`, `user-agent`, or `test-agent`. |
...
| `capabilityTags` | array of string | Optional | OAN capability-domain tags used for governed discovery and filtering. |
...
| `lifecycleState` | string | Optional | Local lifecycle hint such as `draft`, `active`, `suspended`, `revoked`, or `deactivated`. Authoritative lifecycle state is defined by OAN Root and bulletin evidence, not by this hint alone. |
...
When the DID subject is an intelligent agent, software agent, AI service agent, or model-backed agent, `oanMetadata` MAY include an `agentDescription` object.
...
`did:oan` MAY include an `attributes` array for application-facing descriptive metadata.
...
OAN implementations SHOULD use the `cryptoSuite` field on verification methods and proofs when the verification method type alone is not sufficient to select the signing and hashing suite. The following crypto suites are defined for OAN compatibility:
...
The `Sm2Sm3` suite is the OAN profile for SM2 signing with SM3 hashing. A verifier that supports `Sm2Sm3` MUST verify both the `cryptoSuite` value and the verification method key material before accepting a signature. Where `publicKeyJwk` is used for SM2, the JWK SHOULD identify the key type and curve using implementation-compatible values such as `kty: "EC"` and `crv: "SM2"`. Where `publicKeyMultibase` is used, the key encoding profile MUST be sufficient for the verifier to reconstruct the SM2 public key.

Support for `Sm2Sm3` is part of the OAN method profile because OAN deployments may need national-cryptography-compatible signing and verification. A deployment MAY still support Ed25519 for international interoperability, but it SHOULD NOT assume that all conforming `did:oan` documents are Ed25519-only.

### 10.10 Services

The DID Document MAY include service endpoints. For OAN deployments, services commonly include:
...
`did:oan` supports the standard DID lifecycle concepts of creation, update, and deactivation, but specializes them for agent-centric identities.
...
In `did:oan`, the Read operation is realized through DID resolution. In addition to these standard operations, the method also defines method-specific operations such as `Recovery`, `Delegate Capability`, and `Revoke Delegation`.
...
For the `did:oan` method, the DID Method Read operation is realized through DID resolution.

Resolution returns a DID Document and DID document metadata.

Resolvers for `did:oan` SHOULD be capable of returning enough information for a relying party to:
...
`Revoke Delegation` is a method-specific operation of `did:oan`.
...
The following example illustrates an agent-centric `did:oan` DID Document.
...
```json
{
  ...
  "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2",
  "verificationMethod": [
    {
      "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#key-1",
      "type": "Ed25519VerificationKey2020",
      "controller": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2",
      "cryptoSuite": "Ed25519Sha256",
      "publicKeyMultibase": "z6Mkexample"
    },
    {
      "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#recovery-1",
      "type": "Ed25519VerificationKey2020",
      "controller": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2",
      "publicKeyMultibase": "z6Mkrecovery"
    },
    {
      "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#sm2-1",
  ...
  "authentication": [
    "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#key-1"
  ],
  "assertionMethod": [
    "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#key-1"
  ],
  "service": [
    {
      "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#resolver",
      "type": "DIDSubResolver",
      "serviceEndpoint": "https://resolver.OAN.example.org",
      "version": "1.0.0",
      "protocol": "https",
      "serverType": "public",
      "port": 443
    ...
    {
      "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#agent-endpoint",
      "type": "AgentService",
  ...
  "oanMetadata": {
    "subjectType": "agent",
    "nodeRole": "service-agent",
    "identityType": "model-backed-agent",
    "ttl": 86400,
    "recovery": [
      "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#recovery-1"
    ],
    "addressBindings": [
      {
        "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#addr-wallet",
        "addressType": "wallet",
      ...
      {
        "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#addr-domain",
        "addressType": "domain",
        "network": "dns",
    ...
    "delegationChain": [
      {
        "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#delegation-1",
        "delegator": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2",
        "delegate": "did:oan:ORFI:efDelegateExample1234567890",
    ...
    "modelFingerprints": [
      {
        "id": "did:oan:AGFI:efnVUgqQFfYeu97ABf6sGm3WFtVXHZB2#model-1",
...
This specification is intentionally aligned with DID Core and keeps the `did:oan` method focused on method-level semantics.
...
Because `did:oan` can expose service and address bindings, implementations SHOULD protect against stale, malicious, or unauthorized endpoint reassignment.
...
If `did:oan` is submitted to the W3C DID Method Registry, the registration SHOULD emphasize that the method is intended for agent-native identity, service and address identification, delegation-aware control, recovery-aware operations, and optional model-fingerprint-aware identity bindings.

## 20. Conclusion

`did:oan` is a DID method for agent-native identity, not merely a legacy DID document template or an HTTP CRUD interface. Its distinctiveness lies in method-level support for:
